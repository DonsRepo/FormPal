Engineering 478 -- Design with Microcontrollers
...
A Microcontroller Solution to Preventing Exercise Injuries
...
The IMU that was used is a legacy device, so this iteration of the
design utilizes hardware that would otherwise be overlooked. The same
software design could also be carried out in a phone app to further
reduce the additional hardware purchased and eventually discarded by a
user.

Power consumption could be enhanced in this design by taking advantage
of the sleep states of the MCU and IMU hardware. Each of these could
consume significantly less power if they were put to sleep when given
the opportunity. If the button press that triggers the Formpal to enter
into "pretraining" mode was converted to an interrupt, this could be
used to take the MCU out of sleep state. Further, the device could be
put to sleep when in "Exercise mode" when the changes in Euler rotation
and acceleration sits below threshold for long period of time. The IMU
has a low energy pedometer application in its library; this would detect
a large movement which could be used to wake the device out of sleep to
resume exercise activity.
...
![](media/image2.png){width="1.340909886264217in"
